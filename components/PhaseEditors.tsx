    ...
    <div className="space-y-6 pb-10">
      ...
      {activeTab === 'CoEval' && (
         <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 gap-4">{state.coEvaluations.filter(c => !currentUser || c.reviewer === currentUser).map((ev, idx) => (<div key={ev.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 relative"><button onClick={() => removeCoEval(ev.id)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4"/></button><div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center"><div className="md:col-span-4"><label className="text-xs font-bold text-slate-500 uppercase">Evaluado</label><select className="w-full p-2 border rounded bg-slate-50 font-bold" value={ev.target} onChange={(e) => updateCoEval(ev.id, 'target', e.target.value)}><option value="">-- Seleccionar --</option>{config?.members.filter((m: any) => m.name !== currentUser).map((m: any) => (<option key={m.name} value={m.name}>{m.name}</option>))}</select></div><div className="md:col-span-2"><label className="text-xs font-bold text-slate-500 uppercase">Puntos</label><select className="w-full p-2 border rounded font-bold" value={ev.score} onChange={(e) => updateCoEval(ev.id, 'score', parseFloat(e.target.value))}><option value="0">0</option><option value="0.5">+0.5</option><option value="1">+1</option><option value="-0.5">-0.5</option><option value="-1">-1</option></select></div><div className="md:col-span-6"><label className="text-xs font-bold text-slate-500 uppercase">Justificación</label><input className="w-full p-2 border rounded" value={ev.justification} onChange={(e) => updateCoEval(ev.id, 'justification', e.target.value)} /></div></div></div>))}</div>
         </div>
      )}
    </div>
  );
