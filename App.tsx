    ...
    <div className="relative pt-20 pb-16 px-6 border-b border-slate-800">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-900/50 border border-indigo-700 text-indigo-300 text-xs font-bold tracking-wider mb-8 animate-in fade-in slide-in-from-bottom-4">
             <Sparkles className="w-3 h-3" /> <span>NUEVO: GESTIÓN GUIADA POR ROLES</span>
          </div>
          ...
          <p className="text-xl text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed">
             ...
             <span className="text-indigo-400 font-bold">Metodología Flujo Puzle</span>: Cada rol tiene su misión. Fusiona piezas y crea la memoria final.
          </p>
...
}> = ({ currentUser, currentRole, projectState, onChangePhase }) => {
  if (!currentUser) return (
    <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8">
       <UserPlus className="w-16 h-16 mb-4 opacity-20" />
       <h2 className="text-xl font-bold text-slate-600">Identifícate para ver tus tareas</h2>
       <p className="text-sm">Selecciona tu nombre en la barra lateral.</p>
    </div>
  );

  // Metrics Calculation
  const myTrends = (projectState.phases.phase2?.trends || []).filter(t => t.author === currentUser).length;
  const myDishes = (projectState.phases.phase3?.menu || []).filter(d => d.author === currentUser).length;
  const myFinancials = (projectState.phases.phase5?.financials || []).filter(f => {
     const dish = (projectState.phases.phase3?.menu || []).find(d => d.id === f.dishId);
     return dish?.author === currentUser;
  }).length;
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in">
       <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200 flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 text-white flex items-center justify-center text-2xl font-bold shadow-lg">
          ...
          <div className="flex-1">
             ...
             <p className="text-slate-500 font-medium flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${
                   currentRole === RoleType.COORDINATOR ? 'bg-indigo-100 text-indigo-700' :
                   currentRole === RoleType.DOCUMENTATION ? 'bg-emerald-100 text-emerald-700' :
                   currentRole === RoleType.COMMUNICATION ? 'bg-purple-100 text-purple-700' :
                   currentRole === RoleType.RESOURCES ? 'bg-amber-100 text-amber-700' :
                   'bg-pink-100 text-pink-700'
                }`}>{currentRole}</span>
                <span>• Miembro del equipo "{projectState.config?.teamName}"</span>
             </p>
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
             <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><ListTodo className="w-5 h-5 text-indigo-600"/> Mis Tareas Individuales</h3>
             <div className="space-y-4">
                <div>
                   <div className="flex justify-between text-sm mb-1">
                      <span>Tendencias (Fase 2)</span>
                      <span className={myTrends >= 3 ? "text-emerald-600 font-bold" : "text-slate-500"}>{myTrends}/3</span>
                   </div>
                   <div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-indigo-500 h-2 rounded-full transition-all" style={{width: `${Math.min(100, (myTrends/3)*100)}%`}}></div></div>
                </div>
                <div>
                   <div className="flex justify-between text-sm mb-1">
                      <span>Platos Diseñados (Fase 3)</span>
                      <span className={myDishes >= 4 ? "text-emerald-600 font-bold" : "text-slate-500"}>{myDishes}/4</span>
                   </div>
                   <div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-indigo-500 h-2 rounded-full transition-all" style={{width: `${Math.min(100, (myDishes/4)*100)}%`}}></div></div>
                </div>
                <div>
                   <div className="flex justify-between text-sm mb-1">
                      <span>Escandallos Calculados (Fase 5)</span>
                      <span className={myFinancials >= 4 ? "text-emerald-600 font-bold" : "text-slate-500"}>{myFinancials}/4</span>
                   </div>
                   <div className="w-full bg-slate-100 rounded-full h-2"><div className="bg-indigo-500 h-2 rounded-full transition-all" style={{width: `${Math.min(100, (myFinancials/4)*100)}%`}}></div></div>
                </div>
             </div>
             <button onClick={() => onChangePhase('phase3')} className="w-full mt-6 py-2 bg-indigo-50 text-indigo-600 text-sm font-bold rounded-lg hover:bg-indigo-100 transition-colors">Ir a diseñar platos</button>
          </div>

          {/* Role Responsibilities Card */}
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm md:col-span-2">
             <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><Briefcase className="w-5 h-5 text-emerald-600"/> Mis Responsabilidades de Rol</h3>
             <div className="space-y-4">
                {currentRole === RoleType.COORDINATOR && (
                   <div className="p-3 bg-indigo-50 rounded border border-indigo-100 text-sm text-indigo-800">
                      <p className="font-bold mb-1">Fase 1 & Fase 2 (Parte B) & Cronograma</p>
                      <p>Eres el único que puede editar la Configuración y el Modelo de Negocio. También debes liderar el Cronograma en la Fase 4.</p>
                   </div>
                )}
                {currentRole === RoleType.DOCUMENTATION && (
                   <div className="p-3 bg-emerald-50 rounded border border-emerald-100 text-sm text-emerald-800">
                      <p className="font-bold mb-1">Fase 4 (Intro) & Fase 6 (Memoria)</p>
                      <p>Eres responsable de redactar la Introducción en la Fase 4 y de ensamblar toda la Memoria Final en la Fase 6.</p>
                   </div>
                )}
                {currentRole === RoleType.COMMUNICATION && (
                   <div className="p-3 bg-purple-50 rounded border border-purple-100 text-sm text-purple-800">
                      <p className="font-bold mb-1">Fase 3 (Visual) & Fase 6 (Defensa)</p>
                      <p>Debes encargarte del diseño gráfico (Canva, QR) en Fase 3 y preparar los enlaces de la defensa en Fase 6.</p>
                   </div>
                )}
                {currentRole === RoleType.RESOURCES && (
                   <div className="p-3 bg-amber-50 rounded border border-amber-100 text-sm text-amber-800">
                      <p className="font-bold mb-1">Fase 4 (Análisis Sector)</p>
                      <p>Utiliza la investigación de la Fase 2 para redactar un Análisis del Sector sólido en la Fase 4.</p>
                   </div>
                )}
                {currentRole === RoleType.PRODUCTION && (
                   <div className="p-3 bg-pink-50 rounded border border-pink-100 text-sm text-pink-800">
                      <p className="font-bold mb-1">Fase 4 (Diseño) & Fase 5 (Informe)</p>
                      <p>Redacta la Viabilidad Técnica en la Fase 4 y elabora el Informe de Brigada tras la ejecución en la Fase 5.</p>
                   </div>
                )}
                <div className="text-xs text-slate-500 italic mt-4">
                   * Recuerda: Aunque tengas responsabilidades específicas, el trabajo es colaborativo. Ayuda a tus compañeros.
                </div>
             </div>
          </div>
       </div>
...
const SetupConfig: React.FC<{ onComplete: (config: ProjectConfig) => void, onImport: (file: File) => void }> = ({ onComplete, onImport }) => {
  ...
  const isValid = formData.projectName && formData.teamName && formData.zone && formData.members.every(m => m.name.trim().length > 0);
        ...
        <div className="p-8 space-y-10">
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Nombre del Equipo</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Ej: Los Innovadores" value={formData.teamName} onChange={e => setFormData({...formData, teamName: e.target.value})} /></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Número de Grupo</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Ej: G-04" value={formData.groupNumber} onChange={e => setFormData({...formData, groupNumber: e.target.value})} /></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1">Nombre del Proyecto</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Ej: GastroMurcia Experience" value={formData.projectName} onChange={e => setFormData({...formData, projectName: e.target.value})} /></div>
               <div><label className="block text-sm font-bold text-slate-700 mb-1">Fecha de Entrega</label><input type="date" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500" value={formData.deliveryDate} onChange={e => setFormData({...formData, deliveryDate: e.target.value})} /></div>
              <div className="md:col-span-2">
                ...
                {formData.zone && selectedZoneFull && (<div className="mt-2 p-3 bg-indigo-100 text-indigo-800 text-sm rounded border border-indigo-200 flex items-start gap-2"><MapPin className="w-5 h-5 flex-shrink-0 mt-0.5" /><span>{selectedZoneFull}</span></div>)}
              </div>
            </div>
          </section>

          <section className="space-y-6">
            <h3 className="text-xl font-bold text-slate-800 border-b pb-2 flex items-center gap-2"><Users className="w-5 h-5 text-indigo-600"/> Asignación de Roles y Responsabilidades</h3>
                ...
                return (
                  <div key={idx} className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold flex-shrink-0">{idx + 1}</div>
                      <div className="flex-1 space-y-4">
                         <div>
                            ...
                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mb-4"><p className="text-xs font-bold text-slate-500 uppercase mb-2">Responsabilidades Oficiales</p><ul className="list-disc list-inside text-sm text-slate-600 space-y-1">{roleDef?.officialTasks.map((task, tIdx) => (<li key={tIdx}>{task}</li>))}</ul></div>
                         </div>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nombre del Alumno/a <span className="text-red-500">*</span></label><input className="w-full p-2 border border-slate-300 rounded focus:ring-2 focus:ring-indigo-500" placeholder="Nombre completo (Obligatorio)" value={member.name} onChange={(e) => updateMember(idx, 'name', e.target.value)} /></div>
                           <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Responsabilidades Adicionales</label><input className="w-full p-2 border border-slate-300 rounded focus:ring-2 focus:ring-indigo-500" placeholder="Escribe aquí si hay tareas extra..." value={member.tasks} onChange={(e) => updateMember(idx, 'tasks', e.target.value)} /></div>
                         </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="pt-6 border-t flex justify-between items-center">
             <p className="text-sm text-slate-500 italic">{isValid ? "Todo listo para comenzar." : "Rellena todos los campos obligatorios (*)."}</p>
             <button onClick={() => onComplete(formData)} disabled={!isValid} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all"><FileJson className="w-5 h-5" /> Exportar Configuración (JSON)</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-4">
      {parts.map((part, index) => {
        ...
        if (authorMatch) {
          ...
          return (
             <div key={index} className={`${bgColor} ${borderColor} pl-4 py-2 pr-2 my-2 rounded-r-lg text-justify break-inside-avoid`}>
  ...
  <div className="w-72 bg-slate-900 text-slate-300 flex flex-col h-screen fixed left-0 top-0 overflow-hidden shadow-2xl z-40 print:hidden">
    ...
    <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
      <div className="px-4 mb-2">
        <button onClick={() => onChangePhase('dashboard')} className={`w-full text-left p-2 rounded text-sm mb-1 flex items-center gap-3 transition-colors ${activePhase === 'dashboard' ? 'bg-emerald-500/10 text-emerald-400' : 'hover:bg-slate-800'}`}>
           <Home className="w-4 h-4" /> Mi Dashboard
        </button>
        <p className="text-xs font-bold text-slate-500 uppercase mt-4 mb-2">Recursos</p>
        <button onClick={() => onChangePhase('roadmap')} className={`w-full text-left p-2 rounded text-sm mb-1 flex items-center gap-3 transition-colors ${activePhase === 'roadmap' ? 'bg-emerald-500/10 text-emerald-400' : 'hover:bg-slate-800'}`}>
           <Map className="w-4 h-4" /> Guía Didáctica
        </button>
        <button onClick={() => onChangePhase('curriculum')} className={`w-full text-left p-2 rounded text-sm mb-1 flex items-center gap-3 transition-colors ${activePhase === 'curriculum' ? 'bg-emerald-500/10 text-emerald-400' : 'hover:bg-slate-800'}`}>
      ...
      <div className="px-4 mt-6">
       ...
       <div className="px-4 mt-6">
        <button onClick={onPrint} className="w-full p-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-lg font-bold text-sm shadow-lg flex items-center justify-center gap-2 transition-transform hover:scale-105">
...
const App: React.FC = () => {
  ...
  // Derive current role
  const currentRole = projectState.config?.members.find(m => m.name === currentUser)?.role || null;

  // Persistence & Resume Logic (Same as before, robust)
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        const restoredState: ProjectState = {
          ...
          ...parsed.state,
          phases: {
            ...initialProjectState.phases,
            ...(parsed.state.phases || {}),
            phase2: { ...initialProjectState.phases.phase2, ...(parsed.state.phases?.phase2 || {}) },
            phase3: { ...initialProjectState.phases.phase3, ...(parsed.state.phases?.phase3 || {}) },
            phase4: { ...initialProjectState.phases.phase4, ...(parsed.state.phases?.phase4 || {}) },
            phase5: { ...initialProjectState.phases.phase5, ...(parsed.state.phases?.phase5 || {}) },
            phase6: { ...initialProjectState.phases.phase6, ...(parsed.state.phases?.phase6 || {}) },
          }
        };
        // Defensive Arrays
        if (!Array.isArray(restoredState.phases.phase2.trends)) restoredState.phases.phase2.trends = [];
        if (!Array.isArray(restoredState.phases.phase4.timeline)) restoredState.phases.phase4.timeline = [];
        if (!Array.isArray(restoredState.phases.phase5.financials)) restoredState.phases.phase5.financials = [];
        if (!Array.isArray(restoredState.phases.phase6.coEvaluations)) restoredState.phases.phase6.coEvaluations = [];

        setProjectState(restoredState);
        setCurrentUser(parsed.user || '');
        setActivePhase(parsed.phase || 'dashboard');
        setMode(AppMode.WORKSPACE);
      } catch (e) {
        alert("Error al recuperar la sesión anterior.");
      }
    }
  };

  const handleClear = () => { localStorage.removeItem('gastro_project'); setProjectState(initialProjectState); setCurrentUser(''); setMode(AppMode.LANDING); };
  const handleConfigComplete = (config: ProjectConfig) => { setProjectState(prev => ({ ...prev, config })); setMode(AppMode.WORKSPACE); setActivePhase('dashboard'); };
  ...
  const executeSmartMerge = (importedState: ProjectState, author: string | null, isBackup: boolean) => {
    if (isBackup) {
      const restoredState: ProjectState = {
          ...
          phases: {
            ...initialProjectState.phases,
            ...(importedState.phases || {}),
            phase2: { ...initialProjectState.phases.phase2, ...(importedState.phases?.phase2 || {}) },
            phase3: { ...initialProjectState.phases.phase3, ...(importedState.phases?.phase3 || {}) },
            phase4: { ...initialProjectState.phases.phase4, ...(importedState.phases?.phase4 || {}) },
            phase5: { ...initialProjectState.phases.phase5, ...(importedState.phases?.phase5 || {}) },
            phase6: { ...initialProjectState.phases.phase6, ...(importedState.phases?.phase6 || {}) },
          }
      };
      setProjectState(restoredState);
      ...
      if (importedState?.phases?.phase1) newState.phases.phase1 += authorTag + importedState.phases.phase1;
      if (importedState?.phases?.phase2) {
         const p2 = importedState.phases.phase2;
         if (Array.isArray(p2.trends)) newState.phases.phase2.trends = [...newState.phases.phase2.trends, ...p2.trends];
         if (Array.isArray(p2.publicAnalysis)) newState.phases.phase2.publicAnalysis = [...newState.phases.phase2.publicAnalysis, ...p2.publicAnalysis];
         if (Array.isArray(p2.menuBenchmarking)) newState.phases.phase2.menuBenchmarking = [...newState.phases.phase2.menuBenchmarking, ...p2.menuBenchmarking];
         if (p2.synthesis) newState.phases.phase2.synthesis += authorTag + p2.synthesis;
      }
  ...
  const renderPrintView = () => {
    ...
    return (
      <div className="bg-white min-h-screen text-slate-900 print:text-black font-sans">
        <style>{` @page { size: A4; margin: 1.5cm; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .break-after-page { page-break-after: always; } .break-before-page { page-break-before: always; } .avoid-break { page-break-inside: avoid; } `}</style>
        <div className="fixed top-0 left-0 right-0 bg-slate-800 p-4 text-white flex justify-between items-center print:hidden z-50">
           ...
           <div className="flex items-center gap-4"><span className="font-bold">MODO DE IMPRESIÓN:</span><div className="flex bg-slate-700 rounded-lg p-1"><button onClick={() => setPrintMode('partial')} className={`px-4 py-1 rounded ${!isFinal ? 'bg-indigo-500 text-white' : 'text-slate-300'}`}>Memoria Parcial (Fase 4)</button><button onClick={() => setPrintMode('final')} className={`px-4 py-1 rounded ${isFinal ? 'bg-indigo-500 text-white' : 'text-slate-300'}`}>Memoria Final (Fase 6)</button></div></div>
        ...
        <div className="max-w-[210mm] mx-auto pt-20 print:pt-0 bg-white shadow-xl print:shadow-none min-h-[297mm]">
           <div className="h-[297mm] flex flex-col justify-between break-after-page border-b print:border-none p-10 relative">
               <div className="flex flex-col items-center justify-center pt-10">{projectState.config.schoolLogo && (<img src={projectState.config.schoolLogo} className="h-24 w-auto object-contain mb-4" alt="Logo Centro" />)}<h2 className="text-xl font-bold uppercase tracking-widest text-slate-700">{projectState.config.schoolName || "IES LA FLOTA"}</h2><p className="text-sm text-slate-500">{projectState.config.schoolAddress || "Paseo Científico Gabriel Ciscar, nº 1, 30007 Murcia"}</p></div>
           ...
           <div className="px-10 py-10 text-justify leading-relaxed">
              ...
              <section className="mb-10"><h2 className="text-xl font-bold uppercase border-b-2 border-slate-900 mb-6">4. ANÁLISIS</h2><div className="mb-8"><h3 className="font-bold text-lg mb-3 bg-slate-100 p-2 border-l-4 border-indigo-500">Caracterización del sector</h3><AttributionRenderer text={isFinal && phase6?.polishedTexts?.analysis ? phase6.polishedTexts.analysis : (phase4?.sectorCharacterization || "")} config={projectState.config} /></div><div className="mb-8 avoid-break border border-slate-200 rounded p-4 bg-slate-50"><h3 className="font-bold text-lg mb-4 text-center uppercase tracking-widest text-indigo-800">Concepto Propio</h3><div className="grid grid-cols-2 gap-4 text-sm mb-4"><div><span className="font-bold block text-xs uppercase text-slate-500">Nombre:</span> {phase2?.concept?.name}</div><div><span className="font-bold block text-xs uppercase text-slate-500">Tipo:</span> {phase2?.concept?.restaurantType}</div><div className="col-span-2"><span className="font-bold block text-xs uppercase text-slate-500">Descripción:</span> <p className="italic text-slate-600">{phase2?.concept?.description}</p></div></div></div><div className="mb-8"><h3 className="font-bold text-lg mb-3 bg-slate-100 p-2 border-l-4 border-indigo-500">Tendencias</h3>{(phase2?.trends || []).length > 0 ? (<ul className="list-disc pl-5 space-y-2 mb-4">{(phase2.trends || []).map((t, i) => (<li key={i} className="text-sm">{t.description} {t.author && <span className="text-[10px] bg-slate-100 px-1 rounded text-slate-500 ml-2">({t.author})</span>}</li>))}</ul>) : <p className="italic text-slate-400">Sin tendencias.</p>}</div><div className="mb-8"><h3 className="font-bold text-lg mb-3 bg-slate-100 p-2 border-l-4 border-indigo-500">Oferta Gastronómica</h3><AttributionRenderer text={isFinal && phase6?.polishedTexts?.design ? phase6.polishedTexts.design : (phase4?.problemDetected || "")} config={projectState.config} /></div></section>
              <div className="break-after-page" />
              <section className="mb-10"><h2 className="text-xl font-bold uppercase border-b-2 border-slate-900 mb-6">5. DESARROLLO</h2><h3 className="font-bold text-lg mb-2">Planificación</h3><div className="mb-6">{(phase4?.timeline || []).length > 0 ? (<table className="w-full text-sm border-collapse border border-slate-300"><thead><tr className="bg-slate-100"><th className="border p-2">Actividad</th><th className="border p-2">Fechas</th><th className="border p-2">Recursos</th></tr></thead><tbody>{(phase4.timeline || []).map((act, i) => (<tr key={i}><td className="border p-2">{act.activity}{act.author && <span className="block text-[10px] text-slate-400 mt-1">Resp: {act.author}</span>}</td><td className="border p-2">{act.dates}</td><td className="border p-2">{act.resources}</td></tr>))}</tbody></table>) : <p className="italic text-slate-400">Sin cronograma.</p>}</div><h3 className="font-bold text-lg mb-2">Viabilidad</h3><AttributionRenderer text={phase4?.technicalViability || ""} config={projectState.config} /></section>
              <section className="mb-10"><h2 className="text-xl font-bold uppercase border-b-2 border-slate-900 mb-6">6. RESULTADOS</h2><h3 className="font-bold text-lg mb-4">Escandallos</h3>{(phase5?.financials || []).length > 0 ? (<table className="w-full text-sm border-collapse border border-slate-300 mb-6"><thead><tr className="bg-slate-100"><th className="border p-2 text-left">Plato</th><th className="border p-2">Coste Ración</th><th className="border p-2">PVP</th><th className="border p-2">% Food Cost</th></tr></thead><tbody>{(phase5.financials || []).map((f, i) => { const dishName = (phase3?.menu || []).find(d => d.id === f.dishId)?.name || 'Plato'; const costPerRation = (f.totalCost || 0) / (f.numberOfRations || 1); const foodCost = f.sellingPrice > 0 ? (costPerRation / f.sellingPrice) * 100 : 0; return (<tr key={i}><td className="border p-2 font-bold">{dishName}</td><td className="border p-2 text-right">{costPerRation.toFixed(2)}€</td><td className="border p-2 text-right">{f.sellingPrice.toFixed(2)}€</td><td className="border p-2 text-right">{foodCost.toFixed(1)}%</td></tr>)})}</tbody></table>) : <p className="italic text-slate-400 mb-6">Sin datos.</p>}</section>
              <div className="break-after-page" />
              <section className="mb-10"><h2 className="text-xl font-bold uppercase border-b-2 border-slate-900 mb-6">7. CONCLUSIONES</h2><div className="document-font">{isFinal ? phase6?.finalConclusions : "[Pendiente de redacción en Fase 6]"}</div></section>
              <div className="break-after-page" />
              <section><h2 className="text-xl font-bold uppercase border-b-2 border-slate-900 mb-6">ANEXO: CARTA VISUAL</h2><div className="grid grid-cols-2 gap-4">{(phase3?.menu || []).filter(d => d.image).map(d => (<div key={d.id} className="border p-2 rounded break-inside-avoid"><img src={d.image} className="w-full h-48 object-cover rounded mb-2" /><p className="font-bold text-center text-sm">{d.name}</p></div>))}</div>{isFinal && phase6?.teamImage && (<div className="mt-10 break-inside-avoid"><h3 className="font-bold text-lg mb-4 text-center">Equipo del Proyecto: "{projectState.config.teamName}"</h3><img src={phase6.teamImage} className="w-full h-auto rounded-xl shadow-sm border border-slate-200" /></div>)}</section>
              {isFinal && (<section className="mt-10 break-before-page"><div className="border-4 border-red-100 p-8 rounded-xl bg-red-50"><h2 className="text-xl font-bold uppercase text-red-800 mb-2">ANEXO CONFIDENCIAL: COEVALUACIÓN DIABÓLICA</h2><table className="w-full text-sm bg-white border border-red-200"><thead><tr className="bg-red-100 text-red-900"><th className="p-2 text-left">Evaluador</th><th className="p-2 text-left">Evaluado</th><th className="p-2 text-center">Impacto</th><th className="p-2 text-left">Justificación</th></tr></thead><tbody>{(phase6?.coEvaluations || []).map((ev, i) => (<tr key={i} className="border-b border-red-100"><td className="p-2 font-bold">{ev.reviewer}</td><td className="p-2">{ev.target}</td><td className={`p-2 text-center font-bold ${ev.score < 0 ? 'text-red-600' : 'text-emerald-600'}`}>{ev.score.toFixed(2)}</td><td className="p-2 italic text-slate-600">"{ev.justification}"</td></tr>))}</tbody></table></div></section>)}
           </div>
        </div>
      </div>
    );
        ...
        {activePhase === 'print' ? renderPrintView() : (
          <>
            <header className="bg-white h-16 border-b border-slate-200 flex items-center justify-between px-8 shadow-sm z-10">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">{PHASES.find(p => p.id === activePhase)?.title || (activePhase === 'dashboard' ? 'Mi Dashboard' : 'GastroSostenible')}</h2>
              <div className="flex items-center gap-4">{currentUser ? (<span className="px-3 py-1 bg-indigo-50 text-indigo-700 rounded-full text-sm font-bold border border-indigo-100 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></div>{currentUser}</span>) : (<span className="text-red-500 text-sm font-bold animate-pulse flex items-center gap-1"><AlertTriangle className="w-4 h-4" /> Selecciona tu usuario</span>)}</div>
            </header>
            <main className="flex-1 overflow-y-auto p-8 custom-scrollbar relative">
              <div className="max-w-5xl mx-auto h-full">
                 {activePhase === 'dashboard' && <Dashboard currentUser={currentUser} currentRole={currentRole} projectState={projectState} onChangePhase={setActivePhase} />}
                 {activePhase === 'roadmap' && (
                 ...
                 {activePhase === 'phase1' && <Phase1Editor data={projectState.phases.phase1} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase1: d}})} isReadOnly={false} projectContext="" config={projectState.config!} onConfigUpdate={(newConfig) => setProjectState({...projectState, config: newConfig})} currentRole={currentRole} />}
                 {activePhase === 'phase2' && <Phase2Editor data={projectState.phases.phase2} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase2: d}})} projectContext={`Proyecto: ${projectState.config?.projectName}`} currentRole={currentRole} currentUser={currentUser} />}
                 {activePhase === 'phase3' && <Phase3Editor data={projectState.phases.phase3} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase3: d}})} projectContext="" currentUser={currentUser} />}
                 {activePhase === 'phase4' && <Phase4Editor data={projectState.phases.phase4} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase4: d}})} projectContext="" currentRole={currentRole} currentUser={currentUser} />}
                 {activePhase === 'phase5' && <Phase5Editor data={projectState.phases.phase5} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase5: d}})} projectContext="" phase3Data={projectState.phases.phase3} currentUser={currentUser} />}
                 {activePhase === 'phase6' && <Phase6Editor data={projectState.phases.phase6} onUpdate={(d) => setProjectState({...projectState, phases: {...projectState.phases, phase6: d}})} projectContext="" currentUser={currentUser} config={projectState.config!} fullProjectData={projectState} currentRole={currentRole} />}
              </div>
            </main>
          </>
        )}
